...
document.addEventListener('DOMContentLoaded', function() {
    ...
    });    // Loading states for forms (exclude specific forms that handle their own loading)
    const submitButtons = document.querySelectorAll('button[type="submit"]');
    submitButtons.forEach(function(button) {
        button.addEventListener('click', function(e) {
            // Skip if button has data-no-loading attribute
            if (this.hasAttribute('data-no-loading')) {
                return;
            }
            
            const form = this.closest('form');
            // Skip if this is a form that handles its own loading state
            if (form && (form.id === 'loginForm' || form.id === 'contact-form')) {
                return;
            }
